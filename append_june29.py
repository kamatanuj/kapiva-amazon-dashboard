...
with open(f'kapiva_data_{today_str}.json', 'r') as f:
    data = json.load(f)
with open(f'kapiva_data_{prev_str}.json', 'r') as f:
    prev = json.load(f)

products = data['products']
prev_map = {p['asin']: p for p in prev['products']}

prices = [int(p['price']) if isinstance(p['price'], (int, float)) else int(str(p['price']).replace(',', '').replace('₹', '').split('.')[0]) for p in products]
...
for p in products:
    ...
    if pr:
        new = int(p['price']) if isinstance(p['price'], (int, float)) else int(str(p['price']).replace(',', '').replace('₹', '').split('.')[0])
        old = int(pr['price']) if isinstance(pr['price'], (int, float)) else int(str(pr['price']).replace(',', '').replace('₹', '').split('.')[0])
        change = new - old
...
if price_changes:
    changes_html = '''
                    <div style="margin-top:20px; padding:15px; background:#f8fafc; border-radius:8px; border:1px solid #e2e8f0;">
                        <h4 style="margin:0 0 10px 0; color:#334155;">📊 Day-over-Day Price Changes (June 29 vs June 27)</h4>
                        <table class="detail-table" style="font-size:0.9rem;">
                            <thead>
                                <tr><th>Product</th><th>June 27 Price</th><th>June 29 Price</th><th>Change</th><th>% Change</th><th>Trend</th></tr>
                            </thead>
                            <tbody>'''
...
card = f'''<!-- Day: June 29, 2026 (Latest) -->
        <div class="day-card">
            <div class="day-header day-jun29">
                <span><i class="fas fa-star mr-2"></i>June 29, 2026 - Morning Scrape</span>
            ...
            <div class="day-body">
                ...
                <div class="product-details">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                        <h3>All Products Details (June 29, 2026)</h3>
...
comparison_row = f'''<tr style="background: #fce7f3;">
                            <td><strong>June 29, 2026</strong><br/><span style="font-size:0.75rem;color:#999;">Latest</span></td>
                            <td>{len(products)}</td>
                            <td class="price">₹{avg_price}</td>
                            <td class="rating">{avg_rating}★</td>
                            <td>{fmt_reviews(total_reviews)}</td>
                            <td>Apify Amazon Scraper</td>
                            <td><a class="text-purple-600" download="" href="kapiva_data_{today_str}.json">JSON</a> | <a class="text-blue-600" download="" href="kapiva_data_{today_str}.csv">CSV</a> | <a class="text-green-600" download="" href="kapiva_data_{today_str}.xlsx">Excel</a></td>
                        </tr>'''

with open('index.html', 'r') as f:
    html = f.read()

css_addition = '                .day-jun29 { background: linear-gradient(135deg, #059669 0%, #065f46 100%); }\n'
if '.day-jun29' not in html:
    html = html.replace('.day-jun4 {', css_addition + '        .day-jun4 {')

if '<!-- Day: June 29, 2026 (Latest) -->' not in html:
    html = html.replace('<!-- Comparison Table -->', '<!-- Day: June 29, 2026 (Latest) -->\n<!-- Comparison Table -->', 1)

html = html.replace('<tr style="background: #fce7f3;">', '<tr>', 1)

if 'June 29, 2026' not in html:
    html = html.replace(
...
if html.count('day-jun29') < 2:
    html = html.replace(
        '<!-- Day: June 27, 2026 (Latest) -->',
        '<!-- Day: June 29, 2026 (Latest) -->' + '\n' + card + '\n' + '<!-- Day: June 27, 2026 (Latest) -->',
        1
    )

html = re.sub(
    r'<p>Kapiva Dashboard \| Data Source: Amazon\.in \| Updated: June \d{1,2}, 2026</p>',
    '<p>Kapiva Dashboard | Data Source: Amazon.in | Updated: June 29, 2026</p>',
    html
)
